...
def start_ffmpeg_to_hls(source_url: str, output_name: str):
    """Jalankan FFmpeg untuk ubah stream ke HLS (langsung jalan)"""
    output_path = os.path.join(BASE_HLS_DIR, output_name)
    os.makedirs(output_path, exist_ok=True)
    ...
    print(f"[INFO] Start streaming: {source_url}")
...
def convert_stream():
    """Daftarkan stream dan langsung mulai generate HLS"""
    ...
    # Kalau belum aktif, langsung start FFmpeg
    if stream_id not in active_streams:
        ...
        thread = Thread(target=start_ffmpeg_to_hls, args=(link, stream_id))
        thread.daemon = True
        thread.start()
...
def play_stream(stream_id):
    """Player HTML untuk menonton hasil HLS"""
    if stream_id not in active_streams:
        return f"<h2>Stream ID {stream_id} tidak ditemukan.</h2>", 404

    output_path = os.path.join(BASE_HLS_DIR, stream_id, "index.m3u8")
    for _ in range(10):
        if os.path.exists(output_path):
            break
        time.sleep(1)
