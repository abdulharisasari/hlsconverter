...
# Hanya simpan satu stream aktif
active_stream = {
    "id": "",
    "source": "",
    "process": None,
...
def start_ffmpeg_to_hls(source_url: str, output_name: str):
    ...
    # Kalau proses berhenti, hapus folder
    if os.path.exists(output_path):
        shutil.rmtree(output_path, ignore_errors=True)
    active_stream["id"] = ""
    active_stream["source"] = ""
    active_stream["process"] = None
    print("[FFMPEG] Stream stopped & folder cleaned.")


@app.route("/convertStream", methods=["POST"])
def convert_stream():
    """Daftarkan stream baru (jika ada yang lama, hentikan dulu)"""
    ...
    # hentikan stream lama jika masih jalan
    ...
    active_stream["id"] = stream_id
    active_stream["source"] = link
    active_stream["started"] = datetime.now()
    active_stream["process"] = None
...
def play_stream(stream_id):
    """Mainkan stream dan mulai ffmpeg jika belum jalan"""
    ...
    # Start FFmpeg jika belum jalan
    if not active_stream["process"] or active_stream["process"].poll() is not None:
        cleanup_old_stream()
        thread = Thread(target=start_ffmpeg_to_hls, args=(active_stream["source"], stream_id))
        thread.daemon = True
        thread.start()
    ...
    if not os.path.exists(hls_file):
        return "<h2>Menyiapkan stream... tunggu beberapa detik.</h2>", 503
