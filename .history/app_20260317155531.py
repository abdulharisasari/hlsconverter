...
@app.route("/livestream/is_video/<token>")
def is_video(token):
    try:
        # Ambil streamingURL dari API tanpa generate token baru
        ...
        # Cek apakah URL terlihat seperti video (hanya pakai ekstensi)
        is_video = False
        if streaming_url:
            video_ext = (".m3u8", ".mp4", ".mov", ".ts")
            is_video = streaming_url.lower().endswith(video_ext)

        # Kembalikan hasil true/false + URL
        return jsonify({"is_video": is_video, "streamingURL": streaming_url})

    except Exception as e:
        # Jika gagal panggil API → anggap false
