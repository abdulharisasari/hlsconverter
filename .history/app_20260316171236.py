...
def play_camera(token):
    ...
    try:
        ...
        # DEBUG: check whether the streaming_url returns JSON or video content-type
        if streaming_url and isinstance(streaming_url, str) and streaming_url.startswith("http"):
            try:
                head_resp = session.head(streaming_url, timeout=(5, 10), allow_redirects=True, verify=False)
                ctype = head_resp.headers.get("Content-Type")
                status = head_resp.status_code
                print(f"[DEBUG] streaming_url HEAD status={status} Content-Type={ctype}")

                # If the URL returns an error status, or appears to be JSON, try to peek into it.
                if status >= 400 or (ctype and "application/json" in ctype.lower()):
                    try:
                        get_resp = session.get(streaming_url, timeout=(5, 10), stream=True, verify=False)
                        # read a small chunk but don't fetch entire stream
                        peek = get_resp.raw.read(1024) or b""
                        body_preview = peek[:512]
                        print(f"[DEBUG] streaming_url response status={get_resp.status_code} content-type={get_resp.headers.get('Content-Type')} preview={body_preview!r}")

                        # If it looks like JSON, try parse and surface the error message
                        if get_resp.headers.get("Content-Type", "").lower().startswith("application/json"):
                            try:
                                json_body = get_resp.json()
                            except Exception:
                                json_body = None
                            if json_body and isinstance(json_body, dict) and json_body.get("status") == "error":
                                return f"<h2>Stream URL returned error</h2><pre>{json_body}</pre>", 500

                        if get_resp.status_code >= 400:
                            return f"<h2>Stream URL returned HTTP {get_resp.status_code}</h2><pre>{body_preview.decode(errors='replace')}</pre>", 500

                        get_resp.close()
                    except Exception as e:
                        print(f"[DEBUG] failed to fetch streaming_url preview: {e}")
            except Exception as e:
                print(f"[DEBUG] unable to HEAD streaming_url: {e}")
