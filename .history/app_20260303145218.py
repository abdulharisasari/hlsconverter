...
def play_camera(token):
    ...
    try:
        ...
        return jsonify({
            "cameraId": camera_id,
            "streamingUrl": f"{request.host_url}livestream/iOS/{token}?cameraId={camera_id}"
        })
    except Exception as e:
        return f"<h2>Gagal ambil streamingURL: {e}</h2>", 500
...
<script>
    ...
    function startTokenRefresh() {{
        setInterval(async () => {{
            try {{
                const res = await fetch("/generateLinkIOS?cameraId=" + cameraId);
                const data = await res.json();

                if (data.streamingUrl) {{
                    console.log("Token refreshed");

                    // Ambil token baru dari URL
                    const newUrl = data.streamingUrl;

                    // Ganti source tanpa reload page
                    startPlayer(newUrl);
                }}
            }} catch (e) {{
    ...
    if (currentSrc.startsWith("http") && currentSrc.includes(".m3u8")) {{
        startPlayer(currentSrc);
        startTokenRefresh();
    }} else {{
        checkReady();
