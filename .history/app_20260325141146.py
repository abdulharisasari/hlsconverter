...
def get_ffmpeg_quality(viewers):
    if viewers >= 5:
        return ["-c:v", "libx264", "-preset", "veryfast", "-b:v", "1500k"]
    elif viewers >= 2:
        return ["-c:v", "libx264", "-preset", "veryfast", "-b:v", "800k"]
    else:
        return ["-c", "copy"]
...
def run_ffmpeg_to_hls(source_url, stream_id):
    ...
    for _ in range(MAX_RETRY_FFMPEG):
        ...
        viewers = active_streams.get(stream_id, {}).get("viewers", 1)
        quality = get_ffmpeg_quality(viewers)
    ...
    while True:
        for stream_id in list(active_streams.keys()):
            if is_stream_stale(stream_id):
                print("[WATCHDOG RESET]", stream_id)
...
def clean_idle_streams(max_idle=10):
    while True:
        now = time.time()
...
def play(token):
    ...
    <body style="margin:0;background:black;display:flex;align-items:center;justify-content:center;height:100vh;flex-direction:column;">
        ...
        <div id="loading">
            <div style="color:white;">Preparing stream...</div>
        </div>
        ...
        async function start(){
            ...
            window.addEventListener("beforeunload", () => {
                navigator.sendBeacon("/leave-stream/" + streamId);
            });
