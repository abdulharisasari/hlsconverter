...
def debug_stream_response(url):
    try:
        ...
        # bukan JSON → sample text
        try:
            print("TEXT SAMPLE:")
            print(r.text[:300])
        except:
            print("BINARY DATA (video stream)")
...
def play_camera(token):
    try:
        ...
        # ❌ JSON → error API
        if stream_type == "json":
            return f"<h2>Stream ERROR (JSON): {debug_data}</h2>"

        # ❌ request gagal
        if stream_type == "error":
            return f"<h2>Request gagal: {debug_data}</h2>"
        ...
        if is_hls(streaming_url):
            print("DIRECT HLS → PLAY LANGSUNG")
        ...
        # =========================
        # ✅ CASE 2: VIDEO (MP4 / RTSP / dll)
        # =========================
        else:
            print("VIDEO STREAM → CONVERT HLS (FFMPEG)")
