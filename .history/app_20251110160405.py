...
EXPIRE_MINUTES = 1         # hapus setelah 5 menit

# Dictionary: stream_id -> { source, time, is_played }
active_streams = {}
...
def remove_old_streams():
    """Hapus stream yang tidak pernah diputar"""
    now = datetime.now()
    for stream_id, info in list(active_streams.items()):
        age_minutes = (now - info["time"]).total_seconds() / 60

        # hanya hapus kalau belum pernah diputar dan sudah >5 menit
        if not info.get("is_played") and age_minutes > EXPIRE_MINUTES:
            folder = get_stream_folder(stream_id)
            try:
                if os.path.exists(folder):
                    shutil.rmtree(folder)
                    print(f"[CLEANUP] Hapus stream {stream_id} (tidak pernah diplay)")
