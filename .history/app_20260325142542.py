...
def is_stream_stale(stream_id, max_delay=15):
    folder = get_stream_folder(stream_id)
...
def clean_idle_streams(max_idle=30):
    while True:
        ...
        for stream_id, info in list(active_streams.items()):
            ...
            # hanya hapus kalau BENAR-BENAR tidak ada viewer
...
def play(token):
    ...
    <body style="margin:0;background:black;display:flex;align-items:center;justify-content:center;height:100vh;flex-direction:column;">
        ...
        <div id="loading">⏳ Menyiapkan stream...</div>
        ...
        async function start() {
            ...
            async function check() {
                ...
                if (d.ready) {
                    ...
                    loading.innerText = "▶️ Memutar video...";
                    showVideo();
        ...
        // 🔥 kurangi viewer saat keluar
        window.addEventListener("beforeunload", () => {
            if (streamId) {
                fetch("/leave-stream/" + streamId);
            }
        });
