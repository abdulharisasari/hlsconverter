...
CLEANUP_INTERVAL = 60      # cek setiap 1 menit
EXPIRE_MINUTES = 2         # hapus jika tidak diakses > 2 menit
...
# ==============================
# ROUTE ROOT → redirect ke Swagger
# ==============================
@app.route("/")
def index():
    ...
    'parameters': [
        {
            ...
            'type': 'string',
            'required': True,
            'description': 'URL stream sumber'
        }
    ],
    'responses': {
        200: {'description': 'Stream conversion started'},
        400: {'description': 'Parameter link wajib diisi'}
    }
})
def convert_stream():
    data = request.get_json(silent=True) or request.form
...
@swag_from({
    'tags': ['Stream'],
    'responses': {200: {'description': 'Daftar stream aktif'}}
})
def list_streams():
    base_url = request.host_url.rstrip("/")
    return jsonify([
...
@swag_from({
    ...
    'responses': {204: {'description': 'Last access updated'}}
})
def ping_stream(stream_id):
    if stream_id in active_streams:
        active_streams[stream_id]["last_access"] = datetime.now()
