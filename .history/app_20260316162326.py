...
if not resp:
    ...
    <body>
        <div>
            ...
            <p>Tidak ada response dari server kamera</p>
        </div>
    </body>
    </html>
    """
...
def validate_streaming_url(url: str) -> (bool, str):
    """Check if the streaming URL returns a video-related response.
    ...
    try:
        ...
        # common HLS / video content types
        if any(ct in ctype for ct in ["video/", "application/vnd.apple.mpegurl", "application/x-mpegurl", "application/mpegurl"]):
            return True, ""
...
def request_with_retry(url, retries=3, timeout=30):
    for attempt in range(retries):
        try:
            ...
            print(f"[RETRY] Timeout attempt {attempt+1}/{retries}")

        except requests.exceptions.RequestException as e:
            print(f"[RETRY] Error attempt {attempt+1}/{retries}: {e}")
...
def play_camera(token):
    ...
    try:
        ...
        # debug: print API response for inspection
        print(f"[DEBUG] EmbedStaticLink(token={token}) response: {json_data}")

        if not streaming_url:
            return (f"<html><body><h2>streamingURL tidak ditemukan</h2>"
                    f"<pre>{json_data}</pre></body></html>"), 500

        # verify streamingURL is returning video/HLS content before proceeding
        ok, reason = validate_streaming_url(streaming_url)
        if not ok:
            return (f"<html><body><h2>Streaming URL tidak valid</h2>"
                    f"<p>{reason}</p>"
                    f"<p><strong>{streaming_url}</strong></p>"
                    f"</body></html>"), 500
...
def renew_stream(token):
    ...
    try:
        ...
        # debug: print API response for inspection
        print(f"[DEBUG] renew_stream EmbedStaticLink(token={token}) response: {json_data}")

        if not streaming_url:
            return jsonify({"error": "streamingURL tidak ditemukan", "streamingUrl": None}), 500

        ok, reason = validate_streaming_url(streaming_url)
        if not ok:
            return jsonify({"error": f"streamingUrl tidak valid: {reason}", "streamingUrl": streaming_url}), 500
        ...
        if camera_id:
            try:
                ...
                resp2 = requests.get(api_url2, timeout=30, verify=False)
