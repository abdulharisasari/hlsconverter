...
# Logging (suitable for running as a service / via nssm)
...
def remove_old_streams():
                ...
                if os.path.exists(folder):
                    ...
                    for i in range(attempts):
                        try:
                            ...
                            logging.info(f"[CLEANUP] folder {stream_id} masih terkunci, retry {i+1}")
                            time.sleep(1)

                active_streams.pop(stream_id, None)
                logging.info(f"[CLEANUP] Stream {stream_id} dihapus")

            except Exception as e:
                logging.error(f"[CLEANUP ERROR] {stream_id}: {e}")
    ...
    try:
            ...
            if age_minutes > EXPIRE_MINUTES:
                try:
                    ...
                    for i in range(attempts):
                        try:
                            shutil.rmtree(folder)
                            logging.info(f"[CLEANUP] Orphan folder {name} dihapus")
                            break
                        except Exception as e:

                            if i == attempts - 1:
                                logging.error(f"[CLEANUP ERROR] orphan {name}: {e}")
                            else:
                                time.sleep(1)
        ...
        logging.error(f"[CLEANUP ERROR] scanning folders: {e}")
...
def generate_token(camera_id):
    ...
    resp = requests.get(api_url, timeout=60, verify=False)
    resp.raise_for_status()
    logging.info(f"[API] {api_url} -> {resp.status_code} {resp.text}")
...
def play_camera(token):
    ...
    try:
        ...
        logging.info(f"[STREAM URL] token={token} streaming_url={streaming_url}")
        ...
        logging.error(f"[API ERROR] {api_url} -> {status} {getattr(resp, 'text', '')}")
        if status == 400:
    ...
    <script>
        ...
        async function checkReady() {{
            ...
            try {{
                # console.log('[STREAM] checking ready for', streamId, 'src=', src);
                const res = await fetch("/stream-ready/" + streamId);
                const data = await res.json();
                # console.log('[STREAM] /stream-ready ->', data);

                if (data.ready) {{
                    playerStarted = true;
                    hideLoading();
                    console.log('[STREAM] ready -> starting player with', src);
                    startPlayer(src);
                }} else {{
                    setTimeout(checkReady, 1000);
                }}
            }} catch (e) {{
                console.log('[STREAM] error checking readiness', e);
                setTimeout(checkReady, 1000);
            }}
        }}
    ...
    try:
        ...
        if camera_id:
            try:
                ...
                resp2 = requests.get(api_url2, timeout=60, verify=False)
                resp2.raise_for_status()
                logging.info(f"[API] {api_url2} -> {resp2.status_code} {resp2.text}")
                json2 = resp2.json()
