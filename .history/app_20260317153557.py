...
@app.route("/livestream/debug/<token>")
def debug_stream(token):
    """
    Endpoint untuk debug streamingURL tanpa HTML player.
    Menampilkan URL atau pesan error langsung di browser
    dan print ke console.
    """
    try:
        ...
        json_data = resp.json()
        print(f"[DEBUG] Full API response: {json_data}")
        ...
        if streaming_url:
            return f"<h2>Streaming URL ditemukan:</h2><p>{streaming_url}</p>"
        else:
            return "<h2>Streaming URL tidak ada / kosong</h2>", 404

    except requests.exceptions.HTTPError as e:
        return f"<h2>HTTP Error: {e}</h2>", 500
    except requests.exceptions.RequestException as e:
        return f"<h2>Request Error: {e}</h2>", 500
    except Exception as e:
        return f"<h2>Unexpected Error: {e}</h2>", 500
