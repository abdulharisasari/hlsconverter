            ...
            async function doRenew() {{
                try {{
                    ...
                    if (data && data.token) {{
                        ...
                        token = data.token; // update outer token variable as well
                        try {{
                            history.replaceState(null, '', '/livestream/iOS/' + data.token);
                        }} catch (e) {{ /* ignore */ }}
                    }}
                }} catch (e) {{
...
def renew_stream(token):
    ...
    try:
        ...
        # compute stream id from token_used and, if needed, start conversion
