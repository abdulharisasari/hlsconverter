    ...
    try{
        ...
        if(data.status === "ready"){
            ...
            if(retryCount >= MAX_RETRY){
                console.log("[PLAYER] Stream tidak tersedia setelah 3 retry");
                showOffline();
            } else {
                console.log(`[PLAYER] Stream belum siap, retry ${retryCount}/${MAX_RETRY}`);
        ...
        retryCount++;
        if(retryCount >= MAX_RETRY){
            console.log("[PLAYER] Stream tidak tersedia setelah 3 retry:", e);
            showOffline();
        } else {
            console.log(`[PLAYER] Error ping, retry ${retryCount}/${MAX_RETRY}:`, e);
            setTimeout(checkReady, 1000);
        }
    }
}
...
@app.route("/")
def hello():
    return "HLS Converter is running version 2.0 "
...
def play_camera(token):
    try:
        ...
        if not streaming_url:
            print(f"[OFFLINE] Token {token} tidak ada video / stream offline")
            return render_template(
                ...
                fallback_url="",
                is_offline=True,
                error_message="Stream offline atau tidak tersedia."
            )
