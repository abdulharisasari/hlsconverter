...
def generate_link_ios():
    ...
    try:
        ...
        return jsonify({
            "cameraId": camera_id,
            "token": token,
            "streamingUrl": f"${BASE_API}/api/Viewlivestream/iOS/{token}"
        })
...
# ==============================
# ENDPOINT STREAM (ALUR TIDAK DIUBAH)
# ==============================
...
def play_camera(token):
    ...
    # ===============================
    # Jika bukan HLS → convert (TIDAK DIUBAH)
    # ===============================
    else:
        if stream_id not in active_streams:
            thread = Thread(
    ...
    # ===============================
    # HTML PLAYER + 🔥 AUTO REFRESH TOKEN
    # ===============================
    html = f"""
    <!DOCTYPE html>
    <html>
    ...
    <body style="background:black;display:flex;justify-content:center;align-items:center;height:100vh;">
        <video id="video" controls autoplay muted style="width:80%;max-width:900px;"></video>
        ...
        <script>
            ...
            setInterval(async () => {{
                try {{
                    const res = await fetch("/generateLinkIOS?cameraId=REPLACE_CAMERA_ID");
                    const data = await res.json();
                    if (data.token) {{
                        loadStream("/livestream/iOS/" + data.token);
                    }}
                }} catch(e) {{
