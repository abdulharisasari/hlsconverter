...
def run_ffmpeg_to_hls(source_url: str, stream_id: str):
    ...
    cmd += [
        "-rw_timeout", "10000000",  # ⬅️ anti hang
        "-reconnect", "1",
        "-reconnect_streamed", "1",
        "-reconnect_delay_max", "5",
        "-i", source_url,
...
def play_camera(token):
    ...
    try:
        ...
        if not streaming_url:
            return "<h2>Streaming URL tidak tersedia (kamera offline?)</h2>", 500
    ...
    # ===============================
        ...
        if (
            ...
            or active_streams[stream_id].get("proc") is None
or (
    active_streams[stream_id].get("proc") 
    and active_streams[stream_id]["proc"].poll() is not None
)
        ):
        # create active_streams entry before starting ffmpeg thread
    ...
    try:
        ...
        if camera_id:
            try:
                ...
                resp2 = session.get(api_url2, timeout=3, verify=False)
        ...
        if not is_hls(streaming_url):
            if stream_id not in active_streams:
                ...
                if active_streams[stream_id].get("proc") is None:
