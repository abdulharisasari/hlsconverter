...
def cleanup_streams():
    while True:
        ...
        for stream_id in list(active_streams.keys()):
            last_access = active_streams[stream_id]
...
@app.route("/")
def root():
    return "Token Only Streaming Server Running"
...
def generate_link_ios():
    ...
    try:
        ...
        return jsonify({
            "cameraId": camera_id,
            "token": token,
            "streamingUrl": f"{BASE_STREAM_SERVER}/livestream/iOS/{token}"
        })
...
def livestream_ios(token):
    ...
    if is_hls(streaming_url):
        ...
        active_streams[stream_id] = datetime.now()
        hls_url = f"/static/hls/{stream_id}/index.m3u8"
