...
def start_ffmpeg_to_hls(source_url: str, output_name: str):
    """Jalankan FFmpeg untuk ubah source ke HLS dengan kualitas stabil 720p"""
    output_path = os.path.join(BASE_HLS_DIR, output_name)
    os.makedirs(output_path, exist_ok=True)
    ...
    cmd = [
        ...
        "-y",
        "-i", source_url,
        "-vf", "scale=-2:720",         # ubah resolusi ke 720p (proporsional)
        "-c:v", "libx264",             # codec video paling stabil
        "-preset", "veryfast",         # cepat tanpa delay besar
        "-tune", "zerolatency",        # cocok untuk live
        "-c:a", "aac",                 # codec audio umum
        "-b:v", "2500k",               # bitrate video stabil
        "-b:a", "128k",                # bitrate audio
        "-f", "hls",
        "-hls_time", "4",              # segmen 4 detik
        "-hls_list_size", "5",         # simpan 5 segmen terakhir
...
def play_stream(stream_id):
    ...
    <body>
        ...
        <script>
            ...
            // Tambahkan delay kecil agar segmen awal siap
