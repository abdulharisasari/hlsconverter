...
def check_stream_valid(url):
    try:
        ...
        # ✅ kalau JSON → ambil ORI dict
        if "application/json" in content_type:
            try:
                json_data = r.json()  # 🔥 ini asli, bukan string
...
if not is_valid:
    ...
    return render_template(
        ...
        is_offline=True,
        error_json=error_json  # ✅ kirim JSON asli
    )
        if not is_valid:
            print("❌ Stream tidak valid")
            ...
            return render_template(
                ...
                fallback_url=streaming_url,
                is_offline=True,
                error_message=error_msg or "Stream tidak tersedia"
            )
