...
# ==============================
# STREAMING (FLOW LAMA TIDAK DIUBAH)
# ==============================
...
def play_camera(token):
    ...
    <body>
        ...
        <script>
            ...
            function loadStream(tokenValue) {{
                # const src = "/livestream/iOS/" + tokenValue + "?_=" + Date.now();
                const src = "{hls_url}";
            ...
            setInterval(async () => {{
                try {{
                    const res = await fetch("/generateLinkIOS?cameraId={request.args.get('cameraId','')}");
