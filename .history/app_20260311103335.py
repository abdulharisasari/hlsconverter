...
# allow the ffmpeg binary location to be overridden by environment or
# configuration; default to a sensible Windows path but fall back to just
# "ffmpeg" so the script works on other platforms if ffmpeg is on PATH.
...
def remove_old_streams():
    ...
    # Additionally, scan filesystem for orphaned HLS folders that are not tracked
    try:
        for name in os.listdir(BASE_HLS_DIR):
            folder = os.path.join(BASE_HLS_DIR, name)
            if not os.path.isdir(folder):
                continue
            # skip folders currently tracked in memory
            if name in active_streams:
                continue

            # use modification time as a fallback signal of inactivity
            try:
...
def close_stream(stream_id):
    if stream_id in active_streams:
        active_streams[stream_id]['viewers'] = max(0, active_streams[stream_id].get('viewers', 0) - 1)
        active_streams[stream_id]['last_access'] = datetime.now()
    return "", 204
