...
def run_ffmpeg_to_hls(source_url, stream_id):
    ...
    for _ in range(MAX_RETRY_FFMPEG):
        ...
        cmd += [
            ...
            "-f", "hls",
            "-hls_time", "2",
            "-hls_list_size", "10",  # simpan lebih banyak segmen
            "-hls_segment_filename", os.path.join(output_dir, "seg_%03d.ts"),
            "-hls_flags", "append_list+omit_endlist",  # hapus delete_segments
            output_file
        ]
...
def clean_idle_streams(max_idle=30):
    while True:
        ...
        for stream_id, info in list(active_streams.items()):
            ...
            # 🔥 kalau masih ada viewer → refresh stream
            if viewers > 0 and now - last > max_idle:
                print("[AUTO REFRESH]", stream_id)
                refresh_stream(stream_id)
                continue

            # 🔥 kalau tidak ada viewer → hapus
...
def refresh_stream(stream_id):
    ...
    print("[REFRESH STREAM]", stream_id)
    ...
    print("[REFRESH FAILED]", stream_id)
...
def play(token):
    ...
    <body style="margin:0;background:black;display:flex;align-items:center;justify-content:center;height:100vh;flex-direction:column;">
        ...
        async function start() {
            ...
            setInterval(() => {
                if (streamIdGlobal) {
                    fetch("/stream-ready/" + streamIdGlobal)
