...
def play_camera(token):
    ...
    try:
        ...
        print(f"[DEBUG] data0 keys: {list(data0.keys())}")
        if not streaming_url:
            print(f"[WARNING] streaming_url kosong atau tidak tersedia! data0={data0}")
        else:
            print(f"[INFO] streaming_url: {streaming_url}")

        # ambil camera_id jika ada
        camera_id = None
        for k in ("cameraId", "cameraID", "camera_id", "CameraId"):
            if k in data0:
                camera_id = data0.get(k)
        ...
        return f"<h2>Gagal ambil streamingURL: {e}</h2>", 500
            ...
            requests.exceptions.ConnectionError):
        return """
        <html><body><h2>Server streaming tidak merespon, mencoba reconnect...</h2></body></html>
        """

    # ===============================
    # Tentukan HLS URL atau convert
    # ===============================
    hls_url = ""
    if streaming_url:
        if is_hls(streaming_url):
            ...
            # jalankan ffmpeg jika belum ada
            if (stream_id not in active_streams
                or active_streams[stream_id].get("proc") is None
                or active_streams[stream_id]["proc"].poll() is not None):
    ...
    <body>
        {"<h2>Video offline. Link: " + streaming_url + "</h2>" if not streaming_url else '<video id="video" controls autoplay muted></video>'}

        <script>
            const video = document.getElementById('video');
            const src = "{hls_url}";
            const streamingUrl = "{streaming_url}";

            if (!streamingUrl) {{
                console.warn("Streaming URL kosong atau offline, cek server log untuk link:", streamingUrl);
                ...
                console.warn("Browser tidak mendukung HLS");
            }}
        </script>
    </body>
    </html>
