    ...
    try:
        ...
        if streaming_url and isinstance(streaming_url, str) and streaming_url.startswith("http"):
            try:
                ...
                print(f"[DEBUG] streaming_url HEAD Content-Type: {ctype}")

                if ctype and "application/json" in ctype.lower():
                    try:
                        # Some servers don't return JSON on HEAD, try a small GET
                        get_resp = session.get(streaming_url, timeout=(5, 10), stream=True, verify=False)
                        get_resp.raise_for_status()
                        chunk = get_resp.raw.read(1024) or b""
                        print(f"[DEBUG] streaming_url JSON preview: {chunk[:512]!r}")
                        get_resp.close()
                    except Exception as e:
                        print(f"[DEBUG] failed to fetch JSON preview: {e}")
            except Exception as e:
