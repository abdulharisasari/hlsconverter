...
def play_camera(token):
    ...
    # HTML PLAYER
    # ===============================
    # allow optional cameraId passed to this player so the front-end
    # can call /generateLinkIOS to refresh/replace the player source
    camera_id = request.args.get("cameraId", "")
    ...
    <script>
        ...
        function startPlayer(newSrc = null) {{
        ...
        // If a cameraId is provided, periodically request a fresh iOS link
        // and replace the player source with the returned streamingUrl.
        async function renewLink() {{
            if (!cameraId) return;
            try {{
                const res = await fetch('/generateLinkIOS?cameraId=' + encodeURIComponent(cameraId));
                const data = await res.json();
                if (data && data.streamingUrl) {{
                    // Use returned streamingUrl to replace player source
                    startPlayer(data.streamingUrl);
                }}
            }} catch (e) {{
                // ignore errors; we'll retry on next interval
            }}
        }}

        // Renew every 60 seconds while player is active
        if (cameraId) {{
            setInterval(renewLink, 60000);
        }}
    </script>
    </body>
    </html>
