...
def start_ffmpeg_to_hls(source_url: str, output_name: str):
    """Jalankan FFmpeg untuk konversi ke HLS realtime"""
    output_path = os.path.join(BASE_HLS_DIR, output_name)
    os.makedirs(output_path, exist_ok=True)
...
def convert_stream():
    """Daftarkan stream baru dan langsung mulai generate"""
    ...
    # stop stream lama kalau masih aktif
    ...
    # simpan stream baru dan mulai otomatis
    ...
    Thread(target=start_ffmpeg_to_hls, args=(link, stream_id), daemon=True).start()
...
def play_stream(stream_id):
    """Tampilkan player & auto-refresh kalau belum siap"""
    ...
    # tunggu max 30 detik sampai index.m3u8 siap
    ...
    # kalau belum juga, tampilkan halaman "loading"
    ...
    # kalau sudah siap, tampilkan player video
